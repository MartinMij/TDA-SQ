function []=vietoris_rips_javaplexDM(filename,max_dimension,max_filtration_value,...
    number_steps)

% This function is a modification of vietoris_rips_javaplex from Nina Otter 
% (A roadmap for the computation of persistent homology. N. Otter, M. Porter, U. Tillmann,
%  P. Grindrod, H. Harrington, EPJ Data Science 2017 ) in
% order to take as input a general finite metric space (in form of distance matrix)
% instead a point cloud data.
%
% This function computes PH with the VR complex calling javaPlex scripts.
%
% Input: 
% filename: text file with distance matrix
%
...
%%%%Modification to receive distance matrix%%%%%%%
% Load the input file
%%%%point_cloud = load(filename);
dist_mat=load(filename);
filename = regexprep(filename,'.txt',''); 
m_space=metric.impl.ExplicitMetricSpace(dist_mat);
...
stream = api.Plex4.createVietorisRipsStream(m_space, max_dimension,...
     max_filtration_value, number_steps);%%%%% change entry argument
