% This script is intended to obtain a PCA and dispersion plot from Haydn's works as described in the
...
AD=statistics('adagioh');
AL=statistics('allegroh');
AN=statistics('prestoh');
MI=statistics('minuetoh');
X=[AD; AL; AN; MI];
...
legend({'Adagio','Allegro', 'Presto','Menuetto'})
saveas(gcf, 'results/haydn.eps');
%%%dispersion
...
figure
plot([sqrt(sum(DMI)/(size(DMI, 1)-1)), sqrt(sum(DAL)/(size(DAL, 1)-1)), sqrt(sum(DAN)/(size(DAN, 1)-1)),sqrt(sum(DAD)/(size(DAD, 1)-1))]);
xticks([1 2 3 4]);
xticklabels({'Minuet','Allegro','Presto', 'Adagio'});
saveas(gcf, 'results/haydn-dispersion.eps');
